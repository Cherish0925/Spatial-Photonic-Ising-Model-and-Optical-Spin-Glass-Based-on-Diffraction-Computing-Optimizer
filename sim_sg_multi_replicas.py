...
# # ── 1. 生成随机传输矩阵 T（复高斯） ----------------------
# def build_TM(N, M=None, device='cuda'):
#     """返回 T ∈ ℂ^{M×N}，每元素 ~ N(0,1)+i N(0,1)"""
#     M = M or N  # 默认 M = N（全连接）
#     real = torch.randn(M, N, device=device)
#     imag = torch.randn(M, N, device=device)
#     return real + 1j * imag  # 形状 [M, N]
#
#
#
# scattering_matrix = build_TM(N_spin**2, device=device)
# 用 numpy 生成（便于直方图）+ 固定种子保证可复现
seed = 20250608
rng = np.random.default_rng(seed)

N = N_spin * N_spin
real_np = rng.normal(0.0, 1.0, (N, N))
imag_np = rng.normal(0.0, 1.0, (N, N))

# fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 3))

# 实部
# ax1.hist(real_np.ravel(), bins=100, density=True, color='#ff69b4')
# ax1.set_title('Scattering Matrix\'s Real Part')
# ax1.set_xlabel('value'); ax1.set_ylabel('density')
#
# # 虚部
# ax2.hist(imag_np.ravel(), bins=100, density=True, color='#9932cc')
# ax2.set_title('Scattering Matrix\'s Imaginary Part')
...
# 转 torch
scattering_matrix = torch.from_numpy(real_np + 1j * imag_np).to(device, dtype=torch.complex64)
print(f"scattering_matrix.shape={scattering_matrix.shape},scattering_matrix.dtype={scattering_matrix.dtype}")
...
# ==========================================================
# 自旋玻璃哈密顿量 带有自旋构型的平面光入射-衍射传播-无序散射介质-衍射传播-能量探测
# ==========================================================
def sg_hamiltonian(spin, return_intensity=None):
    """
    spin : [N_spin, N_spin] 取值 ±1
    """
    # 1. 把自旋映射到 0/π 相位
    ...
    # 3. 构造复振幅
    field = torch.exp(1j * phase_small)

    # 4. 2F 系统：自由空间→透镜相位→自由空间
    field = prop_f(field.unsqueeze(0) if field.dim()==2 else field)
    # scattering_matrix 左乘
    # 3. 无序介质：展平 → 左乘 T → 还原
    *lead, H, W = field.shape
    N = H * W
    vec = field.reshape(-1, N)
    # print(f"vec.shape={vec.shape},vec.dtype={vec.dtype}")
    out_vec = (scattering_matrix @ vec.T).T  # [B, N] 左乘
    field = out_vec.view(*lead, H, W)  # 还原形状
    # field *= lens_phase[c:c + N_spin, c:c + N_spin]
    field = prop_f(field.unsqueeze(0) if field.dim()==2 else field)

    # print(f"intensity={intensity.shape}")
    # plt.imshow(intensity[0,:,:].cpu().numpy(), cmap='hot')
    # plt.colorbar(label='Intensity (a.u.)')
    # plt.title('2F 后焦面强度')
    # plt.show()

    # 5. 能量
    intensity = (torch.abs(field) ** 2)/(100*100)
    if field.dim() == 2:
        if return_intensity == True:
            return intensity[(50 - delta_x_y):(50 + delta_x_y),
                    (50 - delta_x_y):(50 + delta_x_y)].sum().item(),intensity
        else:
            return intensity[(50 - delta_x_y):(50 + delta_x_y),
                   (50 - delta_x_y):(50 + delta_x_y)].sum().item()
    else:
        if return_intensity == True:
            return intensity[..., (50 - delta_x_y):(50 + delta_x_y),
                    (50 - delta_x_y):(50 + delta_x_y)].sum(dim=(-2, -1)).cpu().numpy(),intensity
        else:
            return intensity[..., (50 - delta_x_y):(50 + delta_x_y),
                   (50 - delta_x_y):(50 + delta_x_y)].sum(dim=(-2, -1)).cpu().numpy()
...
# ==========================================================
def mcmc_ising(spin_coarse_init,steps=100, beta=1.0):
    ...
    spin_coarse_0=spin_coarse_init
    energies= []  # 记录能量和一阶磁化
    spin_full = np.repeat(np.repeat(spin_coarse_0,
                                    scale, axis=1), scale, axis=2)  # (B, N_spin, N_spin)
    old_hamiltonian = -1 * sg_hamiltonian(spin_full)
    ...
    # 预生成随机数
    rng_flip = default_rng()
    for _ in range(steps):
        # i, j = np.random.randint(0, N_coarse, 2)
        b_idx = np.arange(spin_coarse_init.shape[0])
        # print(f"b_idx={b_idx}")
        i = rng_flip.integers(0, N_coarse, size=spin_coarse_init.shape[0])
        j = rng_flip.integers(0, N_coarse, size=spin_coarse_init.shape[0])
        # 备份旧值
        old_vals = spin_coarse_0[b_idx, i, j].copy()
        # print(f"old_vals={old_vals}")
        spin_coarse_0[b_idx, i, j] *= -1
        # 重新构造全图
        spin_full = np.repeat(np.repeat(spin_coarse_0,
                                        scale, axis=1), scale, axis=2)

        new_hamiltonian = -1 * sg_hamiltonian(spin_full)
        delta_E = new_hamiltonian - old_hamiltonian
        # print(f"delta_E={delta_E}")
        # 如果温度下降了，即 delta_E < 0，则直接接受翻转

        # Metropolis 接受概率，逐复本独立
        accept = delta_E <= 0
        reject_mask = ~accept   # 哪些复本需要进一步通过概率判断来决定是否接受翻转
        # print(f"accept={accept},reject_mask={reject_mask}")
        prob = np.exp(-beta * delta_E[reject_mask]) # reject_mask 中为 true 的算出来
        # print(f"accept={accept},reject_mask={reject_mask},prob={prob}")
        # accept[reject_mask] 得到的原本是拒绝翻转的，以 prob 接受
        accept[reject_mask] = rng_flip.random(np.count_nonzero(reject_mask)) < prob
        # 拒绝的复本恢复旧值
        reject_b = b_idx[~accept]
        reject_i = i[~accept]
        reject_j = j[~accept]
        spin_coarse_0[reject_b, reject_i, reject_j] = old_vals[~accept]
        # 更新能量
        old_hamiltonian = np.where(accept, new_hamiltonian, old_hamiltonian)
        energies.append(old_hamiltonian)
    ...
    intensity,intensity_map=sg_hamiltonian(spin_full,return_intensity=True)
    print(f"intensity={intensity.shape}")
    plt.imshow(intensity_map[0,:,:].cpu().numpy(), cmap='hot')
    plt.colorbar(label='Intensity (a.u.)')
    plt.title('Intensity')
    plt.show()

    plt.imshow(spin_coarse_0[0,:,:], cmap='Blues')
    plt.colorbar(label='Spin Orient')
    plt.title('Spin Config')
    plt.show()

    return spin_coarse_0, energies
...
if __name__ == "__main__":
    # 只用一次种子，生成初始构型
    # rng_init = default_rng(42)
    # spin_coarse = 2 * rng_init.integers(0, 2, (replicas,N_coarse, N_coarse)) - 1
    all_energies = []
    # 定义温度范围和步长
    # 定义beta范围和步长
    start_beta = 0.05
    delta_beta = 0.25
    end_beta = 2.55
    beta_list = np.arange(start_beta, end_beta + delta_beta, delta_beta)
    np.random.seed(None)
    final_spin = 2 * np.random.randint(0, 2, (replicas, N_coarse, N_coarse)) - 1
    for beta in beta_list:
        if beta==start_beta:
            final_spin, energy = mcmc_ising(spin_coarse_init=final_spin,steps=steps, beta=beta)
    ...
    step_array = np.arange(steps + 1)

    # 绘制每个复本在每个 beta 下的能量曲线
    for rep_idx in range(replicas):
        for beta_idx, beta in enumerate(beta_list):
            energies = all_energies_array[beta_idx, :, rep_idx]
            # 根据 beta 值选择颜色，beta 越大颜色越暖
            color = plt.cm.hot(beta_idx / len(beta_list))
            ax.plot(step_array, [beta] * len(step_array), energies, color=color)

    # 添加颜色条
    sm = plt.cm.ScalarMappable(cmap=plt.cm.hot, norm=plt.Normalize(vmin=min(beta_list), vmax=max(beta_list)))
    sm.set_array([])
    ...
    ax.set_title('3D Energy Plot for All Replicas and Betas')
